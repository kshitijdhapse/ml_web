import numpy as np
import pickle
import pandas as pd
import streamlit as st 
from PIL import Image
pickle_in = open("clf_entropy.pkl","rb")
clf_entropy=pickle.load(pickle_in)
def welcome():
    return "Welcome All"
def predict_note_authentication(N,P,K,temp,hum,pH,rain):
    prediction=clf_entropy.predict([[N,P,K,temp,hum,pH,rain]])
    print(prediction)
    return prediction
def main():
    st.title("CROP PREDICTOR")
    html_temp = """
    <div style="background-color:tomato;padding:10px">
    <h2 style="color:white;text-align:center;">CROP PREDICTOR</h2>
    </div>
    
    """
    
    # st.markdown("""
    #     <style>
    #     body{
    #         background-image: url('C:/Users/kshit/Documents/ML/nature-product-backdrop-farm-sunlight.jpg');
    #         background-size: cover;
    #         background-position: center;
    #     }
    #     </style>
    #     """,unsafe_allow_html=True)
    
    st.markdown(html_temp,unsafe_allow_html=True)
    st.markdown("""
        <style>
        body{
            background-image: url('C:/Users/kshit/Documents/ML/nature-product-backdrop-farm-sunlight.jpg');
            background-size: cover;
            background-position: center;
        }
        </style>
        """,unsafe_allow_html=True)
    N = st.text_input("N","")
    P = st.text_input("P","")
    K = st.text_input("K","")
    temp = st.text_input("temp","")
    hum = st.text_input("hum","")
    pH = st.text_input("pH","")
    rain = st.text_input("rain","")
    result=""
    if st.button("Predict"):
        result=predict_note_authentication(N,P,K,temp,hum,pH,rain)
    st.success('The output is {}'.format(result))
    if st.button("About"):
        st.text("Lets LEarn")
        st.text("Built with Streamlit")

if __name__=='__main__':
    main()
    



